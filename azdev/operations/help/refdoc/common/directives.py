            ...
            if is_command and help_file.parameters:
               ...
               for arg in sorted(help_file.parameters,
                    ...
                    if arg.deprecate_info:
                        yield '{}:deprecated: {}'.format(self._DOUBLE_INDENT, arg.deprecate_info._get_message(arg.deprecate_info))
                    ...
                    if arg.choices:
                        yield '{}:values: {}'.format(self._DOUBLE_INDENT, ', '.join(sorted([str(x) for x in arg.choices])))
...
def _add_directive(app, name, cls):
    ...
    if name not in directives._directives:
